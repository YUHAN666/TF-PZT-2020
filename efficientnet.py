...
from config import IMAGE_SIZE, ACTIVATION, ATTENTION
from moduleCBAM import moduleCBAM
from squeeze_layers import Squeeze_excitation_layer
...
def mb_conv_block(inputs, block_args, training, keep_dropout_backbone, drop_rate=None, prefix='', ):
    ...
    if block_args.expand_ratio != 1:
        ...
        if ACTIVATION == 'swish':
            x = swish(x, name=prefix + 'expand_swish')
        elif ACTIVATION == 'mish':
            x = mish(x)
        else:
            x = tf.nn.relu(x, name=prefix + 'expand_relu')
    else:
        x = inputs
    ...
    if ACTIVATION == 'swish':
        x = swish(x, name=prefix + '_swish')
    elif ACTIVATION == 'mish':
        x = mish(x)
    else:
        x = tf.nn.relu(x, name=prefix + '_relu')
    ...
    if has_se:
        ...
        if ATTENTION == 'se':
            x = Squeeze_excitation_layer(x, filters, num_reduced_filters, name=prefix + 'se_layer', data_format='channels_last')
        elif ATTENTION == 'cbam':
            x = moduleCBAM(x, filters, num_reduced_filters, name=prefix + 'cbam_layer')
...
def EfficientNet(img_input, input_shape=None, width_coefficient=1.0, depth_coefficient=1.0, default_resolution=224,
                 ...
                 include_top=True, pooling=None, classes=1000, training=True, scope=None, keep_dropout_backbone=True,
                 reuse=None):
