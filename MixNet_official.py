""" Official implementation of MixNet """
...
def build_model_base(images, model_name, training, override_params=None, scope=None):
	...
	with tf.variable_scope(scope):
		model = mixnet_model.MixnetModel(blocks_args, global_params)
		features = model(images, training=training, features_only=True, scope=scope)
		features = tf.identity(features, 'global_pool')

	return [model.endpoints['reduction_1'],
			model.endpoints['reduction_2'],
			model.endpoints['reduction_3'],
			model.endpoints['reduction_4'],
			features]


if __name__ == '__main__':

	image_input = tf.keras.layers.Input(shape=(320, 928, 3))
	features, endpoints = build_model_base(image_input, 'mixnet-s', True, override_params=None, scope='segmentation')
	print(features)
	print('---------------------')
	...
	print('----------------------')
	print(endpoints['block_0'])
	print(endpoints['block_2'])
	print(endpoints['block_6'])
	print(endpoints['block_12'])
	print(endpoints['block_15'])
	for i in endpoints:
		print(i)
